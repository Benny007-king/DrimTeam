...
| `schedule.html` | **מפת משחקים** — לוח שנה (גריד) שבועי (שעות 19:00–23:00, מתרחב לפי הצורך) או חודשי, עם קוביות אירועים לחיצות שמובילות ישר להרשמה, ניווט קדימה/אחורה + "היום", וסינון לפי עיר. משחקי לולאה עתידיים שטרם נפתחו מוצגים כקוביה **אפורה** ("ייפתח בקרוב", לא לחיץ) — והופכים לזמינים ברגע שהלולאה פותחת אותם |
...
| `checkout.html` · `thank-you.html` | תשלום מאובטח (משולם) **או תשלום במזומן במגרש** (הרשמה ללא אשראי) · תודה + כפתור אישור בוואטסאפ |
| `rate.html` | **דירוג מפגש** — סקר כוכבים (1-5) + הערות חופשיות; נשלח **לוואטסאפ הארגון בלבד** (לא לצ'אט האתר). מוצע אוטומטית למשתתפים אחרי סיום מפגש |
| `cards.html` | **כרטיסיות** — שני סוגים: 10 ניקובים ב-₪40/משחק (₪400) ו-10 ניקובים ב-₪45/משחק (₪450). בהרשמה למשחק תואם-מחיר מופיע כפתור **ניקוב** במקום תשלום |
| `stats.html` | **סטטיסטיקות שחקנים** — טבלאות מובילים (נצחונות / הופעות / גולים) + קבוצה מנצחת, עם סינון **מצטבר / חודשי / שבועי**. מתעדכן מהתוצאות שנרשמות בפאנל |
...
| אוסף | תוכן |
|------|------|
| `app/{tournaments,games,recurring,settings,regs}` | נתוני ניהול (`{ value: … }`). `games`/`tournaments`/`recurring` קריאים לכולם; `settings`/`regs` למנהלים בלבד. `recurring` = תבניות לולאת המשחקים (לתצוגת "ייפתח בקרוב") |
...
- **מצב בהיר / כהה** — כפתור בכל הדפים (כולל הפאנל), ברירת מחדל **כהה**, הבחירה נשמרת בקוקי `dt_theme` (גם ללא התחברות).
- **רשימת המתנה** — מספר המקומות הפנוי **מוסתר** מהמשתמשים (נספר מאחורי הקלעים); כשמתמלא נפתחת רשימת המתנה והנרשם רואה את **מיקומו בתור**.
- **גרף פעילות לפי עיר** במפת המשחקים ([schedule.html](schedule.html)) — תרשים עמודות לחיץ שמסנן את הלוח לפי עיר.
- **דירוג מפגשים** — הצעה אוטומטית לדרג מפגש שהסתיים ([rate.html](rate.html)), המשוב נשלח לוואטסאפ הארגון בלבד.
- **סטטיסטיקות שחקנים** ([stats.html](stats.html)) — טבלאות נצחונות/הופעות/גולים + קבוצה מנצחת, סינון מצטבר/חודשי/שבועי, נגזר מ-`results`.
- **כפתור וואטסאפ מהבהב** (לפני התחברות בלבד, 054-5543734); אחרי התחברות מוצג רק הצ'אט.
- **מצב בהיר/כהה** מלא בכל הדפים; בלוח המשחקים מוצג גם **יום בשבוע** בקובייה; **גלריה** מציגה תמונות במלואן בתצוגה המקדימה.
- **כרטיסיות ניקוב** ([cards.html](cards.html)) — נשמרות על מסמך החבר (`members.punches = {"40":n,"45":n}`); בהרשמה למשחק שמחירו תואם לסוג הכרטיסייה מוצג כפתור ניקוב במקום תשלום, עם הצגת היתרה ותזכורת אדומה לחידוש כשנשארים ≤2 ניקובים. כרטיסיית ₪40 תקפה רק למשחקי ₪40 (וכן ₪45).
- **לולאת משחקים קבועה** — תבנית שבועית (יום/שעה/מגרש קבועים); ברגע שהמשחק הנוכחי מסתיים נפתח אוטומטית משחק לשבוע הבא + הודעה לקבוצה. ניתן לבטל שבוע ספציפי (מסומן **"מבוטל"** + הודעת ביטול).
- **תשלום במזומן** — אופציה בהזמנת מקום שמאפשרת הרשמה ללא העברה לתשלום באשראי (התשלום במגרש).
...
- **לולאת משחקים קבועה (🔁)** — הגדרת תבנית חוזרת (יום בשבוע + פרמטרים קבועים); מימוש אוטומטי של המשחק הבא בסיום הקודם (תבניות במפתח הציבורי `app/recurring`, שדות `recurringId`/`status` במשחק), כולל ניהול/ביטול שבועות קרובים, סנכרון עריכות למשחק הפתוח, **שחקנים קבועים** שנרשמים אוטומטית בכל פתיחה (`template.regulars`), והודעות פתיחה/ביטול לקבוצה. עריכת תבנית שמורה משקפת מיד את המשחק הפתוח.
- **חברים**, **חלוקת כוחות** ל-3 קבוצות (לבן/צהוב/כתום) → "סיים ושלח לוואטסאפ" + שליחה לקבוצת הצ'אט; שמירת מצב כוחות.
- **סטטיסטיקות** (מאחד את רישומי התוצאות לשעבר) — בורר משחקים **שהסתיימו בלבד** + סינון "כמה אחורה"; לכל משחק רושמים קבוצה מנצחת + גולים + "ניצח" לכל שחקן (`results/{gameId}` עם `players:[{name,goals,won}]`), וטבלאות מובילים מצטברות. (משחקים שהסתיימו מוסתרים בשאר המקומות בפאנל.)
