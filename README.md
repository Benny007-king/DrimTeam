...
| `schedule.html` | **מפת משחקים** — לוח שנה (גריד) שבועי (שעות 19:00–23:00, מתרחב לפי הצורך) או חודשי, עם קוביות אירועים לחיצות שמובילות ישר להרשמה, ניווט קדימה/אחורה + "היום", וסינון לפי עיר |
...
- **לולאת משחקים קבועה (🔁)** — הגדרת תבנית חוזרת (יום בשבוע + פרמטרים קבועים); מימוש אוטומטי של המשחק הבא בסיום הקודם (`settings.recurring`, שדות `recurringId`/`status` במשחק), כולל ניהול/ביטול שבועות קרובים והודעות פתיחה/ביטול לקבוצה.
