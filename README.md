...
- **לולאת משחקים קבועה (🔁)** — הגדרת תבנית חוזרת (יום בשבוע + פרמטרים קבועים); מימוש אוטומטי של המשחק הבא בסיום הקודם (תבניות במפתח הציבורי `app/recurring`, שדות `recurringId`/`status` במשחק), כולל ניהול/ביטול שבועות קרובים, סנכרון עריכות למשחק הפתוח, והודעות פתיחה/ביטול לקבוצה. עריכת תבנית שמורה משקפת מיד את המשחק הפתוח.
